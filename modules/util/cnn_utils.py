...
def relu(x, leakness=0.0, name=None):
    if leakness > 0.0:
...
def conv(x, filter_size, out_channel, strides, pad='SAME', input_q=None, output_q=None, name='conv'):
    ...
    with tf.variable_scope(name):
        ...
        conv = tf.nn.conv2d(x, kernel, [1, strides, strides, 1], padding=pad)
...
def _add_split_loss(w, input_q, output_q):
    ...
    for i in range(ngroups):
        if len(w.get_shape()) == 4:
            ...
            wg_row = tf.matmul(tf.matmul(tf.diag(tf.square(1 - input_q[i, :])), w_reduce),
                               tf.diag(tf.square(output_q[i, :])))
            wg_row_l2 = tf.reduce_sum(tf.sqrt(tf.reduce_sum(wg_row, 1)))
            wg_col = tf.matmul(tf.matmul(tf.diag(tf.square(input_q[i, :])), w_reduce),
...
def bn(x, is_train, global_step=None, name='bn'):
    """
    batch normalization
    :param x:
    :param is_train:
